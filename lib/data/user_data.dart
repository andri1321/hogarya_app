...
class UserData {
  ...
  static String _userName = "Richie";

  static String _fullName = "pull name";
  ...
  static String userId = "current_user";

  static File? _profileImage;

  static bool verified = true;

  static final List<Property> userPublications = [];

  static final List<Property> allPublications = [];

  static final Set<String> likedPublications = {};

  static final Set<String> favoritePublicationIds = {};
  ...
  static set profileImage(File? value) {
    _profileImage = value;
    _syncUserPublicationOwners();
    _notify();
  }

  static void setInitialPublications(
    List<Property> publications,
  ) {
    final missingPublications = publications.where(
      (publication) => !allPublications.any(
  ...
  static void addPublication(Property property) {
    ...
    userPublications.insert(0, property);
    allPublications.insert(0, property);
    _notify();
  }

  static Property? propertyById(String id) {
    for (final property in allPublications) {
      if (property.id == id) {
        return property;
      }
    }
  ...
  static bool isLiked(String propertyId) {
    return likedPublications.contains(propertyId);
  }

  static bool isFavorite(String propertyId) {
    return favoritePublicationIds.contains(propertyId);
  }

  static List<Property> get favoriteProperties {
    return allPublications
        .where(
          (property) => favoritePublicationIds
              .contains(property.id),
        )
        .toList();
  }

  static void toggleFavorite(Property property) {
    final favorite = favoritePublicationIds.contains(
      property.id,
    );

    if (favorite) {
      favoritePublicationIds.remove(property.id);
    } else {
      favoritePublicationIds.add(property.id);
    }

    final current = propertyById(property.id) ??
        property;
  ...
  static void toggleLike(Property property) {
    ...
    if (liked) {
      likedPublications.remove(property.id);
    } else {
      likedPublications.add(property.id);
    }
  ...
  static List<Map<String, String>> commentsFor(
    String propertyId,
  ) {
  ...
  static void incrementShares(Property property) {
    final current = propertyById(property.id) ??
        property;
  ...
  static String get ownerAvatar =>
      _profileImage?.path ?? networkImage;

  static void _syncUserPublicationOwners() {
    for (var index = 0;
        index < userPublications.length;
        index++) {
  ...
  static Property _withCurrentOwner(
    Property property,
  ) {
  ...
  static void _replacePublication(
    Property property, {
    int? likes,
    int? comments,
    int? shares,
  ...
  static Property _copyProperty(
    ...
    return Property(
      ...
      isFavorite: isFavorite ?? property.isFavorite,
    );
  }
